...
import { STATUS_LABELS, STATUS_COLORS, CERT_LABELS, URGENCY_LABELS, URGENCY_COLORS, buildWhatsAppUrl } from '@/lib/painel/utils';
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex-1 overflow-x-auto">
                <div className="inline-flex h-full min-h-[calc(100vh-4rem)] gap-3 p-4 sm:p-5">
                    {KANBAN_STATUSES.map((status) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`mb-2 flex items-center justify-between rounded-xl border px-3 py-2.5 ${cols.bg} ${cols.border}`}>
                                    <div className="flex items-center gap-2 min-w-0">
                                        <span className={`h-2 w-2 shrink-0 rounded-full ${cols.dot}`} />
                                        <span className={`text-xs font-semibold truncate ${cols.text}`}>
                                            {STATUS_LABELS[status]}
                                        </span>
                                    </div>
                                    <span className={`rounded-full px-2 py-0.5 text-xs font-bold ${cols.text}`}>
                                        {columnLeads.length}
                                    </span>
                                </div>
                                        ...
                                        return (
                                            <div
                                                ...
                                                className={`rounded-xl border border-slate-200 bg-white p-3 shadow-sm transition-all ${
                                                    isDragging ? 'opacity-50 scale-95 cursor-grabbing' : 'cursor-grab hover:shadow-md'
                                                }`}
                                            >
                                                <div className="flex items-start justify-between gap-1 mb-2">
                                                    ...
                                                    <GripVertical size={14} className="shrink-0 mt-0.5 text-slate-300" />
                                                </div>
                                                <p className="text-xs text-slate-500 mb-2">{CERT_LABELS[lead.certificateType]}</p>
                                                <div className="flex items-center justify-between gap-2">
                                                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${urgColor.bg} ${urgColor.text}`}>
                                                ...
                                                {lead.assignedToId && (
                                                    <p className="mt-2 text-xs text-slate-400 truncate">{getUserName(lead.assignedToId)}</p>
                                                )}
                                            </div>
                                        );
                                    })}
